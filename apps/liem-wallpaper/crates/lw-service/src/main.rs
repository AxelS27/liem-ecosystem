...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    init_logging();
    tracing::info!("[Lifecycle] State transitioned to Created");
    
    tracing::info!("[Lifecycle] State transitioned to Initializing");
    ...
    let config_path = lw_core::Config::default_path();
    ...
    tracing::info!("[Lifecycle] State transitioned to Ready");
    ...
    // 6d. Spawn Ecosystem Event Bus background connection loop
    let ecosystem_config = Arc::clone(&config);
    tokio::spawn(async move {
        lw_service::ipc_event_bus::start_ecosystem_ipc_loop(ecosystem_config).await;
    });

    // 6e. Spawn visual Slint Theme HUD window (Disabled per user request)
